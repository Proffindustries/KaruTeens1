...
const AdPerformanceMetrics = ({ performanceData }) => {
    ...
    const totalImpressions = performanceData?.reduce((sum, d) => sum + (d.impressions || 0), 0) || 0;
    ...
    const metrics = [
        { label: 'Impressions', value: totalImpressions.toLocaleString(), icon: <Users />, color: 'blue' },
        { label: 'Clicks', value: totalClicks.toLocaleString(), icon: <TrendingUp />, color: 'green' },
        { label: 'Total Spend', value: `$${totalSpend.toLocaleString()}`, icon: <DollarSign />, color: 'purple' },
        ...
        <div className="performance-metrics-grid">
            {metrics.map((metric, idx) => (
                <div key={idx} className="metric-card card shadow-sm">
                    <div className={`metric-icon color-${metric.color}`}>
                        {metric.icon}
                    </div>
