...
export const fetchTrendingStickers = async (limit = 20, offset = 0) => {
    ...
    try {
        const response = await fetch(`${BASE_URL}/trending?api_key=${GIPHY_API_KEY}&limit=${limit}&offset=${offset}&rating=pg`);
...
export const searchStickers = async (query, limit = 20, offset = 0) => {
    ...
    try {
        const response = await fetch(`${BASE_URL}/search?api_key=${GIPHY_API_KEY}&q=${encodeURIComponent(query)}&limit=${limit}&offset=${offset}&rating=pg`);
        const data = await response.json();
        return data.data.map(sticker => ({
            id: sticker.id,
