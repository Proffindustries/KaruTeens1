...
pub async fn create_ad_campaign_handler(
    ...
    let new_campaign = AdCampaign {
        ...
        start_date: chrono::DateTime::parse_from_rfc3339(&payload.start_date)
            .map_err(|_| AppError::BadRequest("Invalid start date format".to_string()))?.into(),
...
pub async fn list_ad_creatives_handler(
    ...
    let oid = ObjectId::parse_str(&ad_group_id)
        .map_err(|_| AppError::BadRequest("Invalid ad group ID".to_string()))?;
...
async fn build_ad_campaign_response(
    campaign: &AdCampaign,
    state: &Arc<AppState>,
) -> Result<AdCampaignResponse, AppError> {
    // Get groups count
    let groups = state.mongo.collection::<AdGroup>("ad_groups");
...
async fn build_ad_group_response(
    group: &AdGroup,
    state: &Arc<AppState>,
) -> Result<AdGroupResponse, AppError> {
    // Get creatives count
    let creatives = state.mongo.collection::<AdCreative>("ad_creatives");
...
async fn build_ad_creative_response(
    creative: &AdCreative,
    _state: &Arc<AppState>,
) -> Result<AdCreativeResponse, AppError> {
...
pub async fn get_active_ads_handler(
    State(state): State<Arc<AppState>>,
    Query(params): Query<AdFilter>,
) -> AppResult<impl IntoResponse> {
...
pub async fn track_ad_event_handler(
    State(state): State<Arc<AppState>>,
    Json(payload): Json<TrackAdEventRequest>,
) -> AppResult<impl IntoResponse> {
    let creative_oid = ObjectId::parse_str(&payload.creative_id)
        .map_err(|_| AppError::BadRequest("Invalid creative ID".to_string()))?;

    let creatives = state.mongo.collection::<AdCreative>("ad_creatives");
    let analytics = state.mongo.collection::<AdAnalytics>("ad_analytics");
    
    // 1. Fetch the creative to get campaign and group IDs
    let creative = creatives.find_one(doc! { "_id": creative_oid }, None).await
         .map_err(|e| AppError::DatabaseError(e.to_string()))?
        .ok_or_else(|| AppError::NotFound("Creative not found".to_string()))?;

    // 2. Update Creative counts
    let update_field = match payload.event_type.as_str() {
        "impression" => "impressions",
        "click" => "clicks",
        "conversion" => "conversions",
        _ => return Err(AppError::BadRequest("Invalid event type".to_string())),
    };

    creatives.update_one(
        doc! { "_id": creative_oid },
        doc! { "$inc": { update_field: 1 } },
        None
    ).await .map_err(|e| AppError::DatabaseError(e.to_string()))?;

    // 3. Update or Create Analytics record for today
    let today = Utc::now().date_naive().and_hms_opt(0, 0, 0).unwrap();
    let today_bson = mongodb::bson::DateTime::from_millis(today.and_utc().timestamp_millis());

    let filter = doc! {
        "creative_id": creative_oid,
        "date": today_bson
    };
    ...
    let update = doc! {
        "$inc": { update_field: 1 },
        "$set": {
            "campaign_id": creative.ad_group_id, 
            "group_id": creative.ad_group_id,
            "created_at": mongodb::bson::DateTime::now()
        }
    };

    let options = mongodb::options::UpdateOptions::builder().upsert(true).build();
    analytics.update_one(filter, update, options).await
         .map_err(|e| AppError::DatabaseError(e.to_string()))?;

    Ok((StatusCode::OK, Json(json!({"message": "Event tracked"}))))
}

pub fn ad_routes() -> Router<Arc<AppState>> {
    Router::new()
        .route("/", get(list_ad_campaigns_handler).post(create_ad_campaign_handler))
        .route("/tracker", post(track_ad_event_handler))
        .route("/active", get(get_active_ads_handler))
