...
pub async fn list_stories_handler(
    ...
    while let Some(story_res) = cursor.next().await {
        if let Ok(s) = story_res {
            let story_info: crate::features::infrastructure::dto::StoryResponse = build_story_response(&s, &state).await?;
...
pub async fn get_story_handler(
    ...
    let story_info: crate::features::infrastructure::dto::StoryResponse = build_story_response(&story, &state).await?;
