...
pub async fn get_user_posts_handler(
    ...
    let post_responses: Vec<crate::dto::PostResponse> = crate::content::posts_to_responses(&state, None, posts).await;
...
pub async fn follow_user_by_username_handler(
    ...
    if let Ok(Some(follower_prof)) = profiles_collection.find_one(doc! { "user_id": follower_id }, None).await {
        ...
        tokio::spawn(async move {
            let _ = crate::push::notify_new_follower(&state_clone, followed_id, follower_id, fname).await;
        });
    }
